...
const configPath = path.resolve(
  process.cwd(),
  getArg("--config", "guardrails.config.json"),
);
const config = loadConfig(configPath);

const include = Array.isArray(config.include) ? config.include : [];
const exclude = Array.isArray(config.exclude) ? config.exclude : [];
const allowIn = Array.isArray(config.allowIn)
  ? config.allowIn.map(normalizePath)
  : [];
const message =
  config.message ?? "Direct network calls are not allowed in this scope.";
...
console.log(
  `check-no-direct-fetch passed (${candidates.length} files scanned).`,
);
