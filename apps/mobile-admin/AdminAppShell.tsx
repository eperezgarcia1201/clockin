...
export default function App() {
  ...
  const liquorSheetRows = useMemo(
    () => buildLiquorSheetRows(liquorCatalog, liquorCounts),
    [liquorCatalog, liquorCounts],
  );
  ...
  const employeePunchStatus = useMemo(
    () => buildEmployeePunchStatusMap(activeNow),
    [activeNow],
  );
  const managerProfile = useMemo(
    () => selectManagerProfile(employees, sessionManagerEmployeeId),
    [employees, sessionManagerEmployeeId],
  );
  const managerPunchRow = useMemo(
    () => selectManagerPunchRow(recentPunchRows, sessionManagerEmployeeId),
    [recentPunchRows, sessionManagerEmployeeId],
  );

  const managerCurrentPunchStatus = managerPunchRow?.status || "OUT";
  const { managerCanClockOut, managerNextPunchType, managerActionLabel } =
  ...
  useAdminViewEffects({ loggedIn, screen, loadCompanyOrderCatalog, loadCompanyOrders, hasLiquorManagerAccess, loadLiquorControlData, setLiquorCatalog, setLiquorCounts, setLiquorMovements, setLiquorMonthly, setLiquorMonthlyPrevious, setLiquorYearly, setLiquorBottleScans, setLiquorSheetDrafts, setLiquorInvoiceRows, setLiquorInvoiceImageDataUrl, setLiquorInvoiceImageName, setLiquorStatus, companyOrdersOfficeId, liquorMonth, liquorTargetCostPct, liquorYear, scopedLocationId, salesExpenseMethod, setSalesExpenseCheckNumber, setSalesExpensePayToCompany, canManageMultiLocation, activeLocationId, offices, setActiveLocationId, officeGeoTarget, setOfficeGeoLatitude, setOfficeGeoLongitude, setOfficeGeoRadius, setOfficeGeoStatus, liquorInventorySearch, setLiquorInventoryVisibleCount, liquorCatalogSearch, setLiquorCatalogVisibleCount, liquorSheetRows, });

  useVisibleTabSyncEffect({ loggedIn, screen, visibleTabs, setScreen, });
  const adminContentProps = { ...adminState, isLight, text, inlineOrNull, handleLogin, dataSyncError, managerProfile, managerCurrentPunchStatus, managerCanClockOut, handleManagerSelfPunch, inline, managerActionLabel, handleSubmitManagerPendingTips, handleForcePunch, pendingScheduleOverrides, handleScheduleOverrideDecision, formatDisplayDate, loadEmployees, loadActiveNow, loadSummary, handleCreateUser, cancelEditUser, saveUserEdits, employeePunchStatus, loadUserForEdit, handleSetUserDisabled, activeLocationLabel, canManageMultiLocation, scopedLocationId, canCreateLocations, handleCreateOffice, officeGeoTarget, handleSaveOfficeGeofence, handleCreateGroup, saveSalesReport, todayExpenseDate, captureSalesExpenseReceipt, saveSalesExpense, reportTypeOrder, runReport, activeMessageEmployees, sendEmployeeMessage, loadNotifications, parseScheduleOverrideNotification, selectedScheduleEmployee, todayScheduleLabel, loadTodaySchedule, todayRoleTabs, activeTodayRoleFilter, filteredTodayScheduleRows, filteredScheduleEmployees, updateScheduleDay, adjustScheduleTime, saveSchedule, handleCompanyOrderExport, selectedCompanyOrderSupplier, visibleCompanyOrderItems, selectedCompanySupplierDraft, handleCompanyOrderAddItem, hasMoreCompanyOrderItems, selectedCompanyOrderCount, selectedCompanyOrderTotalUnits, companyOrderCartItems, handleCompanyOrderStepItem, handleCompanyOrderRemoveItem, submitCompanyOrder, selectedCompanyOrderSupplierCount, loadCompanyOrders, hasLiquorManagerAccess, hasLiquorPremiumAccess, companyOrdersOfficeId, loadLiquorControlData, liquorSheetRows, updateLiquorSheetDraft, saveLiquorCatalogRow, saveLiquorCountRow, createLiquorKind, deleteLiquorKind, assistLiquorCatalog, lookupLiquorByUpc, createLiquorCatalogItem, createLiquorMovement, saveLiquorQuickCount, analyzeLiquorBottleForItem, pickLiquorInvoicePhoto, analyzeLiquorInvoicePhoto, applyLiquorInvoiceRows, handleLiquorAnalyticsExport, };
