...
export type BottleScanWorkflowResult =
  | { state: "canceled" }
  | { state: "status"; message: string; reload: boolean };
...
export async function captureAndAnalyzeLiquorBottle(params: {
  ...
  const { itemId, officeId, containerKey, fetchJson } = params;

  const permission = await ImagePicker.requestCameraPermissionsAsync();
  if (!permission.granted) {
    return {
      state: "status",
      message: "Camera permission is required for bottle scan.",
      reload: false,
    };
  }
  ...
  return {
    state: "status",
    message: `Bottle scan saved. Fill ${fillPercent}% • Spent ${spentMl} ml.`,
    reload: true,
  };
}
...
export async function pickLiquorInvoicePhotoFromLibrary(): Promise<InvoicePhotoPickResult> {
  ...
  if (!permission.granted) {
    return {
      state: "status",
      message: "Photo library permission is required for invoice OCR.",
    };
  }
