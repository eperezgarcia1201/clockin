...
export class LiquorInventoryController {
  ...
  @Get('kinds')
  async listKinds(@Req() req: RequestWithUser) {
    if (!req.user) {
      throw new UnauthorizedException();
    }
    return this.liquorInventory.listKinds(req.user);
  }

  @Post('kinds')
  async upsertKind(
    @Req() req: RequestWithUser,
    @Body() dto: CreateLiquorKindDto,
  ) {
    if (!req.user) {
      throw new UnauthorizedException();
    }
    return this.liquorInventory.upsertKind(req.user, dto);
  }

  @Delete('kinds/:name')
  async hideKind(@Req() req: RequestWithUser, @Param('name') name: string) {
    if (!req.user) {
      throw new UnauthorizedException();
    }
    return this.liquorInventory.hideKind(req.user, name);
  }
  ...
  @Post('catalog/ai-assist')
  async assistCatalog(
    @Req() req: RequestWithUser,
    @Body() dto: AssistLiquorCatalogDto,
  ) {
    if (!req.user) {
      throw new UnauthorizedException();
    }
    return this.liquorInventory.assistCatalog(req.user, dto);
  }
  ...
  async analyzeInvoice(
    @Req() req: RequestWithUser,
    @Body() dto: AnalyzeLiquorInvoiceDto,
  ) {
    if (!req.user) {
      throw new UnauthorizedException();
    }

    return this.liquorInventory.analyzeInvoice(req.user, dto);
  }

  @Post('invoices/apply')
  async applyInvoiceRows(
    @Req() req: RequestWithUser,
    @Body() dto: ApplyLiquorInvoiceDto,
  ) {
    if (!req.user) {
      throw new UnauthorizedException();
    }

    return this.liquorInventory.applyInvoiceRows(req.user, dto);
  }
