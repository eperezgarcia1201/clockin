...
const toEmail = (name) => `${name.toLowerCase().replace(/[^a-z0-9]+/g, ".").replace(/\.+/g, ".")}` +
    "@clockin.local";
async function main() {
    ...
    const superUserEmail = process.env.SEED_SUPER_USER_EMAIL || "dev@clockin.local";
    ...
    const superUserRole = Object.values(client_1.Role).includes(superUserRoleValue)
        ? superUserRoleValue
        : client_1.Role.OWNER;
    ...
    for (const employee of employees) {
        ...
        const pinHash = await (0, bcryptjs_1.hash)(employee.pin, 10);
